...
@pytest.mark.parametrize("name", ["cam1", "cam-north", "cam_north", "cam.1", "A1"])
...
def test_read_url_embeds_credentials(stack: CameraStack) -> None:
    stack.server.auth = AuthSpec(username="reader", password="p@ss")
    assert stack.read_url(stack.cameras[0]) == "rtsp://reader:p%40ss@127.0.0.1:8554/cam1"
...
def test_legacy_streams_manifest_is_accepted(tmp_path: Path, video_file: Path) -> None:
    path = write_config(
        ...
        {
            "streams": [
                {"name": "cam-day", "source": str(video_file), "offset_seconds": 12},
                {"name": "cam-night", "source": str(video_file), "enabled": False},
            ]
        },
        name="streams.yaml",
    )
    ...
    assert [camera.name for camera in stack.cameras] == ["cam-day", "cam-night"]
...
def test_save_and_reload_preserves_settings(tmp_path: Path, video_file: Path) -> None:
    original = CameraStack(
        server=ServerSpec(rtsp_port=9000, auth=AuthSpec(username="reader", password="s3cr3t")),
        cameras=[
            CameraSpec(
                name="cam1",
                source=video_file,

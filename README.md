# eais-virtual-rtsp-camera

`vcam` — a command line tool that turns local video files into **looping virtual RTSP
cameras**, so DeepStream / AIP pipelines can be developed and tested without physical
cameras.
...
```bash
uv run vcam run --source videos/toll-gate.mp4
# -> rtsp://127.0.0.1:8554/toll-gate
```
...
```bash
uv run vcam run \
  --camera gate-north=videos/overview.mp4 \
  --camera gate-south=videos/plate.mp4 \
  --camera gate-rain=videos/rain.mp4
# -> rtsp://127.0.0.1:8554/gate-north
#    rtsp://127.0.0.1:8554/gate-south
#    rtsp://127.0.0.1:8554/gate-rain
```

With credentials (readers must authenticate, local publishers still don't):

```bash
uv run vcam run -s videos/toll-gate.mp4 --username edgeai --password s3cret
# -> rtsp://edgeai:s3cret@127.0.0.1:8554/toll-gate
```

Read a stream back:

```bash
ffprobe -rtsp_transport tcp rtsp://127.0.0.1:8554/toll-gate
ffplay  -rtsp_transport tcp rtsp://127.0.0.1:8554/toll-gate
```
...
uv run vcam run -s videos/toll-gate.mp4 --mode copy

# force a 720p15 2 Mbit/s H.264 feed regardless of the source
uv run vcam run -s videos/toll-gate.mp4 \
...
```bash
uv run vcam probe videos/toll-gate.mp4
```
...
cameras:
  - name: gate-north           # becomes the RTSP path
    source: videos/overview.mp4  # relative paths resolve next to this file
...
Legacy `jetson/streams.yaml` manifests (`streams:` with `offset_seconds`) are accepted too:

```bash
uv run vcam list -c jetson/streams.yaml
```
...
`jetson/` contains the container-based deployment (MediaMTX + supervisor) used on the
EdgeAI station, and `scripts/transfer-to-jetson.sh` copies media onto the device. On
aarch64 the CLI downloads the `linux_arm64` MediaMTX build automatically. Prefer a
